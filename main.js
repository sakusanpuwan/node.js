...
var logConfig_default = logger;

// src/index.js
var result = import_dotenv.default.config();
if (result.error) {
  logConfig_default.error("\u26A0\uFE0F Could not load .env file", result.error);
  process.exit(1);
}
logConfig_default.info(`The value of PI is ${PI}`);
console.log(`2 + 3 = ${add(2, 3)}`);
console.log(`2 * 3 = ${multiply(2, 3)}`);
console.log(`Name is ${constants_default.name}`);
console.log(`Language is ${constants_default.language}`);
console.log(`Version is ${constants_default.version}`);
logConfig_default.info(`Environment variable: ${process.env.SECRET_KEY}`);
logConfig_default.error("ERROR: This is an error message");
